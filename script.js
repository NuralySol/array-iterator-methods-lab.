...
const totalYearsLived = inventors.reduce((total, inventors) => {
    const yearsLived = inventors.passed - inventors.year;
    return total + yearsLived;
}, 0); //acculimator of the inital value total
...
const inventorNames = inventors.map(inventors => `{first:${inventors.first},  last:${inventors.last}}`);
...
const sortedByBirthYear = inventors.sort((a, b) => a.year - b.passed);
...
const inventorNamedAda = inventors.find(Ada => Ada.first ===
    'Ada' && Ada.last === 'Lovelace'
);
...
const firstLast = people.map(people => {
    const [firstName, lastName] = people.split (',');
...
isAdultPresent = devs.some(devAgeCheck => devAgeCheck.year >= 18);
...
currentYear = new Date().getFullYear(); //Here I used in build Class of Date with Current year method of that class

isEveryone19OrOlder = devs.every(devAgeEveryone => (currentYear - devAgeEveryone)>=19);
...
const idx = comments.findIndex(commentindex => commentindex.id === 123523);
